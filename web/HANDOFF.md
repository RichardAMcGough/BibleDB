...
Clone target: `C:\Work\Resurrected\Claude\BibleDB\LXX-Rahlfs-1935`
(any path works — `import_lxx.py --lxx-root <path>` overrides).
...
`--dry-run` validates the parser without touching the DB. `--lxx-root`
is optional; defaults to `C:\Work\Resurrected\Claude\BibleDB\LXX-Rahlfs-1935`.
