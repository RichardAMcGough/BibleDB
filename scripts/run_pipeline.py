...
def ensure_schema_migrations(cfg: dict, log) -> bool:
    ...
    try:
        ...
        # ── 14: standalone notes SQL parity migration ───────────────────────
        def m14():
            sql_path = PROJECT_ROOT / "sql" / "schema" / "notes_live_migration.sql"
            if not sql_path.exists():
                log("    notes_live_migration.sql not found — skipping")
                return
            log("    applying notes_live_migration.sql ...")
            run_sql_script(sql_path)
        run(14, "notes_live_migration_sql", m14)

        # ── 15: notes write stored procedures (least-privilege web user) ───
        def m15():
            log("    creating notes stored procedures ...")

            cur.execute("DROP PROCEDURE IF EXISTS sp_create_verse_note")
            cur.execute("""
            ...
            cur.execute("DROP PROCEDURE IF EXISTS sp_update_verse_note")
            cur.execute("""
            ...
            cur.execute("DROP PROCEDURE IF EXISTS sp_delete_verse_note")
            cur.execute("""
            ...
            cur.execute("DROP PROCEDURE IF EXISTS sp_set_verse_note_visibility")
            cur.execute("""
            ...
            conn.commit()
        run(15, "notes_write_stored_procs", m15)
